...
func (c *Client) CreateSecret(name, namespace string, secType corev1.SecretType, data map[string]interface{}) error {
	...
	log.Infof("created secret %q in %s namespace", result.GetObjectMeta().GetName(), namespace)

	return nil
}

// UpdateSecret updates a K8s secret with a given name from a given map and creates one if it's absent
func (c *Client) UpdateSecret(name, namespace string, data map[string]interface{}) error {
	secretsClient := c.Clientset.CoreV1().Secrets(namespace)
	...
	log.Infof("updated secret %q in %s namespace", result.GetObjectMeta().GetName(), namespace)

	return nil
}

// ReadSecret reads a K8s secret with a given name
func (c *Client) ReadSecret(name, namespace string) (map[string][]byte, error) {
	secretsClient := c.Clientset.CoreV1().Secrets(namespace)
...
// GetAnnotatedPullSecrets get a K8s pull secret with a given name and annotations
func (c *Client) GetAnnotatedPullSecrets(namespace string, annotations map[string]string) ([]corev1.Secret, error) {
	secretsClient := c.Clientset.CoreV1().Secrets(namespace)
...
// GetSecrets get a K8s secret with a given name and annotations
func (c *Client) GetSecrets(namespace string) ([]corev1.Secret, error) {
	secretsClient := c.Clientset.CoreV1().Secrets(namespace)
